...
#  결제 요청 정보 Validation
  integrityParams: ->
    ...
    try
      throw '결제할 금액을 설정해주세요. ( 100원 이상, 본인인증/정기 결제요청의 경우엔 0원을 입력해주세요. ) [ params: price ]' if (isNaN(price) or price < 100) and (@zeroPaymentMethod.indexOf(@params.method) is -1 or (not @params.method?.length or not @params.pg?.length))
